...
if (len(sys.argv) != 13 and len(sys.argv) != 15):
  ...
  print("or -> python new_blockchain1.py -host 1 (2 ou 3 ou 4) -name <id-new-blockchain> -validator_port 4004 -rest_port 8008 -consensus_port 5050 -network_port 8800 -peers <tcp://ip-validator0:port>")
  exit(0)
...
PEERS_IP       = sys.argv[14].split(',') if HOST_NUMBER != '0' else []#'--seeds tcp://' + sys.argv[14] if len(sys.argv) == 15 else ''

if len(PEERS_IP) != int(HOST_NUMBER):
    print("ERROS: Expected %d peers but only got %d\n",int(HOST_NUMBER), len(PEERS_IP))
    exit(0)
...
entrypoint = "bash -c \" \
        ...
        sawtooth-validator -v \
            ...
            --scheduler parallel \
            --network-auth trust \
      \"".format(HOST_NUMBER, HOST_NUMBER, entrypoint_main if HOST_NUMBER == '0' else '', NETWORK_IP, NETWORK_PORT, VALIDADOR_IP, VALIDADOR_PORT, CONSENSUS_IP, CONSENSUS_PORT, VALIDADOR_IP, VALIDADOR_PORT,'--peers ' + PEERS if HOST_NUMBER != '0' else '')
